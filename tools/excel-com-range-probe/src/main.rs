...
fn run(arguments: Vec<String>) -> Result<(), String> {
    ...
    match command {
        ...
        "automation-value-live-observe" => {
            let (rows, required_failures) = excel_com_range_probe::automation_value_live_observations(
                options.get("case").map(String::as_str),
            )?;
            println!(
                "{}",
                serde_json::to_string(&serde_json::json!({
                    ...
                    "prompt_06_evidence_modified": false,
                }))
...
fn usage() -> String {
    "usage: excel-com-range-probe <live|diagnose|parity|refresh|check|kernel-init|kernel|kernel-check|value-matrix|value-matrix-refresh|value-matrix-check|error-scode|error-scode-refresh|error-scode-python|error-scode-check|automation-value-check|automation-value-live|automation-value-live-observe|python-differential|python-differential-raw|python-differential-refresh|python-differential-check> --root <knowledge-root> [--control-script <path>] [--mode <rust-baseline|pywin32-dynamic|pywin32-generated|comtypes-dynamic|comtypes-generated|L|S|X|all>] [--backend <raw-windows-sys|high-level-windows>] [--action <single|repeatability|compare|retry|scalar-value2>] [--fixture <controlled-fixture>] [--case <case-id>] [--run-id <id>] [--python <path> --client <pywin32|comtypes> --wrapper <dynamic|generated|all> --family <all|mixed|date|shape|dynamic> --environment-id <id> --cache-dir <path>]"
        .to_owned()
}
